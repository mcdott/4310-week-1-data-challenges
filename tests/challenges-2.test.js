...
describe("Challenge 2 Titanic", () => {
  ...
  test("Test getAll", () => {
    ...
    expect(index.getAllValuesForProperty(data, "embarked")).toEqual(
      allEmbarked
    );
  });
  ...
  test.skip("Test sumAllProperty", () => {
    ...
    const sumFares = data.reduce(
      (acc, p) => (p.fields.fare !== undefined ? acc + p.fields.fare : acc),
      0
    );
  ...
  test("Test makeHistogram", () => {
    ...
    expect(index.makeHistogram(data, "age", 10)).toEqual(
      Array.from(ages10, (v) => v || 0)
    );
    expect(index.makeHistogram(data, "age", 5)).toEqual(
      Array.from(ages5, (v) => v || 0)
    );
    expect(index.makeHistogram(data, "fare", 10)).toEqual(
      Array.from(fares, (v) => v || 0)
    );
  });
  ...
  test.skip("Test getUniqueValues", () => {
    ...
    expect(index.getUniqueValues(data, "embarked").sort()).toEqual(
      ["C", "S", "Q", undefined].sort()
    );
    expect(index.getUniqueValues(data, "sex").sort()).toEqual(
      ["male", "female"].sort()
    );
    expect(index.getUniqueValues(data, "survived").sort()).toEqual(
      ["Yes", "No"].sort()
    );
  });
});
